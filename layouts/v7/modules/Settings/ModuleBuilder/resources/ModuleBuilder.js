 ...
 * All Rights Reserved.
 *************************************************************************************/
Vtiger.Class('Settings_ModuleBuilder_Js', {
	...
	triggerAdd : function(event) {
		...
		var instance = Settings_ModuleBuilder_Js.currentInstance;
	...
	//constructor
	init : function() {
		Settings_ModuleBuilder_Js.currentInstance = this;
	},
            ...
            function(err,data) {
                if(err === null) {
                    ...
                    var record = form.find('[name="record"]').val();
						...
						thisInstance.registerEditEvents(form);
                    }else {
	...
	saveRuleDetails : function(form) {
		...
		if(!/^[a-zA-Z0-9]+$/.test(data['name'])) {
			alert(app.vtranslate('LBL_NAME_ERROR'));
			return ;
		}
		if(!/^[,_a-zA-Z0-9]+$/.test(data['linkfield'])) {
			alert(app.vtranslate('LBL_LINKFIELD_ERROR'));
			return ;
		}
	...
	registerEditEvents :function(form) {
		form.find('[name="template"]').on('change', function(){
			$this = $(this);
			var input = form.find('[name="linkfield"]');
			var value = $this.val();
			if(input.val() != "") {
				input.val(input.val() + ',');
			}
