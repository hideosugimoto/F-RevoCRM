...
class Services extends CRMEntity {
	...
	*/
	...
	/**	function used to get the list of payments which are related to the service
	 *	@param int $id - service id
	 *	@return array - array which will be returned from the function GetRelatedList
	 */
	function get_payments($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_crmentity.*,
			...
			LEFT OUTER JOIN vtiger_account
				ON vtiger_account.accountid = vtiger_payment.accountid
			INNER JOIN vtiger_inventoryproductrel
				ON vtiger_inventoryproductrel.id = vtiger_payment.paymentid
			LEFT JOIN vtiger_groups
				ON vtiger_groups.groupid = vtiger_crmentity.smownerid
