...
class Accounts extends CRMEntity {
        ...
        $query = "SELECT
				...
				FROM
					...
					LEFT JOIN vtiger_account 
						ON vtiger_account.accountid = vtiger_purchaseorder.accountid 
				...
				WHERE
					...
					AND vtiger_purchaseorder.accountid =
				".$id;
	...
	*/
	function get_invoices($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		/**
	* Function to get Account related Payments
	* @param  integer   $id      - accountid
	...
	*/
	function get_payments($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
				vtiger_crmentity.*, vtiger_payment.*, vtiger_account.accountname, vtiger_purchaseorder.subject AS purchasesubject
				FROM vtiger_payment
				...
				LEFT OUTER JOIN vtiger_purchaseorder ON vtiger_purchaseorder.purchaseorderid = vtiger_payment.purchaseorderid
				...
				LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid = vtiger_users.id
				WHERE vtiger_crmentity.deleted = 0 AND (vtiger_payment.accountid = $id";

		if(!empty ($entityIds)){
			$query .= " OR vtiger_payment.contactid IN (".$entityIds."))";
		} else {
			$query .= ")";
		}
