...
class Contacts extends CRMEntity {
	...
	var $emailTemplate_defaultFields = array('lastname','firstname','salutation','title','email','department','phone','mobile','support_start_date','support_end_date');
...
// vtiger_account query goes for ecery single vtiger_account row
	...
	*/
	...
	/**
	* Function to get Contact related Payments
	* @param  integer   $id      - contactid
	* returns related Payments record in array format
	*/
	function get_payments($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
			...
			vtiger_payment.*,
			vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
			vtiger_purchaseorder.subject AS purchasesubject
			FROM vtiger_payment
			INNER JOIN vtiger_crmentity
				ON vtiger_crmentity.crmid = vtiger_payment.paymentid
			LEFT OUTER JOIN vtiger_contactdetails
				ON vtiger_contactdetails.contactid = vtiger_payment.contactid
			LEFT OUTER JOIN vtiger_purchaseorder
				ON vtiger_purchaseorder.purchaseorderid = vtiger_payment.purchaseorderid
			LEFT JOIN vtiger_groups
				ON vtiger_groups.groupid = vtiger_crmentity.smownerid
            LEFT JOIN vtiger_paymentcf
                ON vtiger_paymentcf.paymentid = vtiger_payment.paymentid
			LEFT JOIN vtiger_paymentbillads
				ON vtiger_paymentbillads.paymentbilladdressid = vtiger_payment.paymentid
			LEFT JOIN vtiger_paymentshipads
				ON vtiger_paymentshipads.paymentshipaddressid = vtiger_payment.paymentid
			LEFT JOIN vtiger_users
				ON vtiger_crmentity.smownerid = vtiger_users.id
			WHERE vtiger_crmentity.deleted = 0
			AND vtiger_contactdetails.contactid = ".$id;
